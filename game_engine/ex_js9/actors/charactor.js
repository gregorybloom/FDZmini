...
//	Actor.prototype.draw.call(this);
	...
	if(this.hurtTimer.running) {
 	...
 	var prop = {fill:false, color:this.charColor,width:5};
	prop.width -= 4*(1-this.health/this.maxhealth);
...
/*
		if(this.impactTimer.running) {
			...
			var r = (c.time + this.impactTimer.lifeTime)/this.impactTimer.lifeTime;
			GAMEVIEW.fillCircle(pos,this.radius-2,"rgba(255,0,0,"+(r*0.8)+")");
		}
		else if(this.invincTimer.running) {
			var c = this.invincTimer.getCycle();
			var f = (c.time + this.invincTimer.lifeTime)*10;
			f = f % this.invincTimer.lifeTime;
			f = f / this.invincTimer.lifeTime;
			var r = f; //(c.time + this.invincTimer.lifeTime)/this.invincTimer.lifeTime;
			GAMEVIEW.fillCircle(pos,this.radius-2,"rgba(50,50,50,"+(r*0.7)+")");
		}
/**/
		if(this.hurtTimer.running) {
