...
//		console.log(m, bOLine, _Cpt, radius, '==', ptA, ptB);
		var underHAT = quadB*quadB - 4*quadA*quadC;
//		console.log(underHAT,'==', quadA, quadB, quadC);
		if(underHAT > 0) {
...
/*
		if(x2 == x1) && (y2 == y1)
			if getDistance pt to circlept == radius, return true
		fi
		(x-cpt.x)^2 + (y-cpt.y)^2 = radius ^2
		if(x2 != x1) && (y2 != y1)
			m = (y2 - y1)/(x2 - x1)
			b = y - mx		// use any pt
			y = mx + b
		...
		if(x2 != x1) && (y2 != y1)
			(x-cpt.x)^2 = radius*radius - (mx+b)^2





	//		x=tx1+(1−t)x2,			t = (x1 - x2) / (x - x2)

/*		var dr = {x:0,y:0};
		dr.x = linePt2.x - linePt1.x;
		dr.y = linePt2.y - linePt1.y;

		var fr = {x:0,y:0};
		fr.x = linePt1.x - circlept.x;
		fr.y = linePt1.y - circlept.y;

		var lineLength = GAMEGEOM.getDistance(linePt2, linePt1);

		var D = Math.sqrt( P.x*P.x + P.y*P.y );
		if(D > lineLength+radius)		return false;

		var P1t2 = {x:0,y:0};
		P1t2.x = circlept.x - linePt2.x;
		P1t2.y = circlept.y - linePt2.y;

		var D1t2 = P1t2.x*P1t2.x + P1t2.y*P1t2.y;
		D1t2 = Math.floor(D1t2);
		if(D1t2 < radius*radius) {
			return {pt2:P2,d:D};
		}
		return false;
/*
